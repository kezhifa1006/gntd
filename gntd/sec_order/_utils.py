...
class ComputeA:
    ...
    @classmethod
    def compute_mat_a(cls, a, layer):
        return cls.__call__(a, layer)

    @classmethod
    def __call__(cls, a, layer):
        if isinstance(layer, nn.Linear):
            mat_a = cls.linear(a, layer)
        elif isinstance(layer, nn.Conv2d):
            mat_a = cls.conv2d(a, layer)
        else:
            # FIXME(CW): for extension to other layers.
            # raise NotImplementedError
            mat_a = None
...
class ComputeG:
    ...
    @classmethod
    def compute_g(cls, g, layer, batch_averaged=False):
        """
        :param g: gradient
        :param layer: the corresponding layer
        :param batch_averaged: if the gradient is already averaged with the batch size?
    ...
    def __call__(cls, g, layer, batch_averaged):
        if isinstance(layer, nn.Conv2d):
            mat_g = cls.conv2d(g, layer, batch_averaged)
        elif isinstance(layer, nn.Linear):
            mat_g = cls.linear(g, layer, batch_averaged)
        else:
            mat_g = None
