...
# from _utils import (ComputeCovA, ComputeCovG, ComputeMatGrad)
...
class EKFACOptimizer(optim.Optimizer):
    def __init__(self,
        ...
        self.MatGradHandler = _utils.ComputeMatGrad()
    ...
    def _update_scale(self, m):
        with torch.no_grad():
            ...
            # if self.steps == 0:
            #     self.S_l[m] = s_l.new(s_l.size()).fill_(1)
            _utils.update_running_stat(s_l, self.S_l[m], self.stat_decay)
